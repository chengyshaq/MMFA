...
class Train_MMFA_Model(object):

    def __init__(self, mmfa_model, train_data_loader, epoch, optimizer, show_epoch,
                 loss_coefficient, model_save_epoch, model_save_dir):
        self.mmfa_model = mmfa_model
    ...
    def train(self, fold):
        ...
        for epoch in range(self.epoch):
            self.mmfa_model.train()
            for step, train_data in enumerate(self.train_data_loader):
                inputs, labels = train_data
                self.optimizer.zero_grad()
                results = self.mmfa_model(inputs, True, labels)
            ...
            if (epoch + 1) % self.model_save_epoch == 0:
                torch.save(self.mmfa_model.state_dict(),
